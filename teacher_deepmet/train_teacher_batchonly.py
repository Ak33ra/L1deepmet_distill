...
parser.add_argument('--ckpts', default='/hildafs/projects/phy230010p/share/NanoAOD/Znunu/ckpts_znunu_Sep17_teacher_batchonly',
...
if __name__ == '__main__':
    ...
    print(f"Training with batch size: {args.batch_size} (ONLY change from original)")

    dataloaders = data_loader.fetch_dataloader(data_dir=args.data, 
                                               batch_size=args.batch_size,  # Only change
                                               validation_split=.25)
    ...
    print(f"Estimated time per epoch: {len(train_dl) * 1.0 / 60:.1f} minutes")
    print(f"Total estimated time: {len(train_dl) * 1.0 * max_epochs / 3600:.1f} hours")
