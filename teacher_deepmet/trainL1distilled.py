...
from evaluateKyungmin import evaluate
import warnings
...
parser.add_argument('--student_ckpts', default='student_ckpts_L1',
                    help="Name of student ckpts folder")
parser.add_argument('--teacher_ckpts', default='teacher_ckpts_L1',
                    help="Name of teacher ckpts folder")
...
def train(model, device, optimizer, scheduler, loss_fn, dataloader):
    ...
    with tqdm(total=len(dataloader)) as t:
        for data in dataloader:
            ...
            teacher_x_cont = data.x[:,:n_features_cont].clone()
            teacher_x_cat = data.x[:,n_features_cont:].long().clone()
            ...
            with torch.no_grad():
                teacher_result = teacher(teacher_x_cont, teacher_x_cat, edge_index, data.batch)
            
            loss = loss_fn(result, data.x, data.y, data.batch)
            loss_d = F.mse_loss(result, teacher_result)
            a = 0.3
            # convex combination of normal and distillation loss
            loss = a * loss + (1 - a) * loss_d
            loss.backward()
            optimizer.step()
...
if __name__ == '__main__':
    ...
    # teacher model
    teacher = net.Net(n_features_cont, n_features_cat, norm).to(device)
    teacher_ckpt = torch.load(os.path.join(args.teacher_ckpts, 'best.pth.tar'))
    ...
    # output dir
    model_dir = osp.join(os.environ['PWD'],args.student_ckpts)
    ...
    # teacher dir
    teacher_dir = osp.join(os.environ['PWD'],args.teacher_ckpts)
