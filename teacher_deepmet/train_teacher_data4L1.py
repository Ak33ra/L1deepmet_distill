...
if __name__ == '__main__':
    ...
    for epoch in range(first_epoch+1, max_epochs+1):
        ...
        if is_best:
            print('Found new best loss at epoch', epoch, ':', validation_loss) 
            ...
            # Also save as best_teacher.pth.tar for student training
            utils.save_checkpoint({'epoch': epoch,
                                   'state_dict': model.state_dict(),
                                   'optim_dict': optimizer.state_dict(),
                                   'sched_dict': scheduler.state_dict()},
    ...
    loss_log.close()
    print('Training completed! Best validation loss:', best_validation_loss)